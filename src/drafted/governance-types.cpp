...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "MCTNetwork") type = MCTNetwork;
    if(strType == "MCTNetworkVariable") type = MCTNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == MCTNetwork) s = "MCTNetwork";
    if(type == MCTNetworkVariable) s = "MCTNetworkVariable";
